...
xxxxxxxxxx typedef struct{    int value;    struct process_control_block *list;​}semaphores;wait(semaphores *S){    S->value--;    if (S->value<0) block(S->list);}signal(semaphore *S){    S->value++;    if(S->value<=0) wakeup(S->list);}c

## 解题思路
### 暴力
xxxxxxxxxx class Solution {public:    inline int mymax(int a,int b){        return a>b?a:b;    }    string longestCommonPrefix(const vector<string>& strs) {        string res="";        char cur;        int m_lenth=0;        for(const auto &s:strs){            m_lenth=mymax(m_lenth,s.length());        }        res.reserve(m_lenth);        for(int i=0;i<m_lenth;i++){            for(int j=0;j<strs.size();j++){                if(!j&&i){                    res.push_back(cur);                }                if(strs[j].size()<=i){                                        return res;                }                else{​                    if(!j){                        cur=strs[j][i];                    }                    else{                        if(cur!=strs[j][i]){                            return res;                        }                    }                }            }        }        res.push_back(cur);        return res;            }};c++
```c++
class Solution {
public:
